...
var GridstackService = /** @class */ (function () {
    ...
    GridstackService.prototype.detachGridItemIfExists = function (gridItemId) {
        var /** @type {?} */ gridItem = this._gridItems.find(function (gi) { return gi.item.generatedId == gridItemId; });
        if (!!gridItem) {
...
var GridstackComponent = /** @class */ (function () {
    ...
    GridstackComponent.prototype.ngOnInit = function () {
        ...
        this._zone.runOutsideAngular(function () {
            _this_1._renderer.addClass(_this_1._el.nativeElement, 'grid-stack');
            if (_this_1._isValueTrue(_this_1.animate)) {
                _this_1._renderer.setAttribute(_this_1._el.nativeElement, 'data-gs-animate', 'true');
            }
            _this_1._setAttributeIfNotUndefined('data-gs-width', _this_1.width);
            _this_1._setAttributeIfNotUndefined('data-gs-height', _this_1.height);
            var /** @type {?} */ width = _this_1.width || (_this_1.options || {}).width;
            if (!_.isNull(width) && !_.isUndefined(width)) {
                _this_1._renderer.addClass(_this_1._el.nativeElement, "grid-stack-" + width);
            }
            var /** @type {?} */ el = ( /** @type {?} */($(_this_1._el.nativeElement))).gridstack(_this_1.options);
            $(el).data('generated-id', _this_1.generatedId);
            _this_1._gridstack = $(el).data('gridstack');
            _this_1._gridstackService.addGrid(_this_1);
            // Hook events
            $(_this_1._el.nativeElement).on('added', function (evt, items) {
                var /** @type {?} */ itemsOfCurrentGrid = _this_1._gridstackService.getGridItems(_this_1.generatedId);
                var /** @type {?} */ existingItemIds = ( /** @type {?} */(_this_1._gridstack)).grid.nodes.map(function (i) { return $(i.el)
                    .attr('class')
                    .split(/\s+/)
                    .find(function (s) { return s.startsWith('lb-generated-id-'); })
                    .replace('lb-generated-id-', ''); });
                var /** @type {?} */ itemsToAdd = existingItemIds.filter(function (i) { return !itemsOfCurrentGrid.some(function (ei) { return ei.generatedId == i; }); });
                itemsToAdd.forEach(function (i) { return _this_1._gridstackService.attachGridItem(_this_1.generatedId, i); });
                _this_1.added.emit(items);
            });
            $(_this_1._el.nativeElement).on('change', function (evt, items) {
                _this_1.change.emit(items);
            });
            $(_this_1._el.nativeElement).on('removed', function (evt, items) {
                var /** @type {?} */ itemsOfCurrentGrid = _this_1._gridstackService.getGridItems(_this_1.generatedId);
                var /** @type {?} */ existingItemIds = ( /** @type {?} */(_this_1._gridstack)).grid.nodes.map(function (i) { return $(i.el)
                    .attr('class')
                    .split(/\s+/)
                    .find(function (s) { return s.startsWith('lb-generated-id-'); })
                    .replace('lb-generated-id-', ''); });
                var /** @type {?} */ itemsToDelete = itemsOfCurrentGrid.filter(function (i) { return !existingItemIds.some(function (ei) { return ei == i.generatedId; }); });
                itemsToDelete.forEach(function (i) { return _this_1._gridstackService.detachGridItemIfExists(i.generatedId); });
                _this_1.removed.emit(items);
            });
        });
    };
    /**
    ...
    GridstackComponent.prototype.ngAfterViewInit = function () {
        ...
        this.gridstackItems.changes
            ...
            _this_1._handleItemChanges(_this_1.gridstackItems.toArray());
        });
    ...
    GridstackComponent.prototype._updateGridstackHeight = function (height) {
        var _this_1 = this;
        this._zone.runOutsideAngular(function () {
            _this_1._gridstack.cellHeight(/** @type {?} */ (height));
        });
    };
    /**
     * @param {?} items
    ...
    GridstackComponent.prototype._handleItemChanges = function (items) {
        var _this_1 = this;
        var /** @type {?} */ itemsOfCurrentGrid = this._gridstackService.getGridItems(this.generatedId);
        var /** @type {?} */ itemsToAdd = items.filter(function (i) { return !itemsOfCurrentGrid.some(function (w) { return w.generatedId == i.generatedId; }); });
        var /** @type {?} */ itemsToRemove = itemsOfCurrentGrid.filter(function (w) { return !items.some(function (i) { return i.generatedId == w.generatedId; }); });
        this._gridstack.batchUpdate();
        itemsToAdd.forEach(function (i) { return _this_1._addItem(i); });
        itemsToRemove.forEach(function (i) { return _this_1._removeItem(i); });
    ...
    GridstackComponent.prototype._addItem = function (item) {
        var _this_1 = this;
        this._zone.runOutsideAngular(function () {
            if (_this_1._gridstack.willItFit(+item.x, +item.y, +item.width, +item.height, _this_1._isValueTrue(item.autoPosition))) {
                _this_1._gridstackService.addOrphanGridItem(item);
                $(item.elem.nativeElement).addClass('lb-generated-id-' + item.generatedId);
                _this_1._gridstack.makeWidget(item.elem.nativeElement);
            }
            else {
    ...
    GridstackComponent.prototype._removeItem = function (item) {
        ...
        this._zone.runOutsideAngular(function () {
            _this_1._gridstack.removeWidget(item.elem.nativeElement);
        });
    };
    /**
     * @param {?} attrName
     ...
     * @return {?}
     */
    GridstackComponent.prototype._setAttributeIfNotUndefined = function (attrName, val) {
        if (!_.isNull(val) && !_.isUndefined(val)) {
